/**
 * Nacimientos: barras nacionales (tab_nacional) + tabla municipal (porc_naci_2024_redo → JSON por_naci_2024_redo).
 */
...
function maxNaci(states) {
  let m = 1;
  for (const s of states || []) {
    const v = Number(s.naci_24);
...
function stateBarRow(nomEnt, naci24, maxV, gue) {
  const w = maxV > 0 ? Math.round((Number(naci24) / maxV) * 100) : 0;
  ...
  track.append(fill);
  const num = el("div", "nacim-state-num", [document.createTextNode(fmtMiles(naci24))]);
 ...
 * @param {any} payload
 */
export function renderNacimientosVista(root, payload) {
  if (!root) return;
  root.innerHTML = "";
  ...
  if (!payload || !payload.ok) {
    ...
    root.append(el("div", "nacimientos-viz-error", [document.createTextNode(msg)]));
    return;
  }

  const states = payload.states || [];
  const maxV = maxNaci(states);
  const porEnt = payload.por_entidad_guerrero;

  const wrap = el("div", "nacimientos-viz");

  wrap.append(
    el("h3", "nacimientos-viz-title", [
  ...
  for (const s of states) {
    natBody.append(stateBarRow(s.nom_ent, s.naci_24, maxV, !!s.estatal_si));
  }
  ...
  tbl.append(
    el("div", "nacim-tbl-row nacim-tbl-row--head", [
      ...
      el("div", "nacim-tbl-h", [document.createTextNode("Porcentaje de nacimientos")]),
    ])
  );
  ...
  tbl.append(
    tblSectionTitle("Municipios con mayor participación porcentual de nacimientos")
  );
  for (const r of payload.top5 || []) {
    tbl.append(tblDataRow(r.nom_mun, r.por_naci_2024_redo, !!r.highlight, false));
  }
  ...
  if (payload.middle) {
    ...
    mid.append(
      tblDataRow(
        ...
        payload.middle.por_naci_2024_redo,
        false,
        true
      )
    );
  ...
  tbl.append(
    tblSectionTitle("Municipios con menor participación porcentual de nacimientos")
  );
